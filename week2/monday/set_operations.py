...
jan_customers.union(feb_customers).show()

# TODO 1b: Union January and February, then remove duplicates
jan_feb_unique = jan_customers.union(feb_customers).distinct()
jan_feb_unique.show()

# TODO 1c: Try union with March customers - what happens?
# Use unionByName to fix it
jan_customers.union(feb_customers).unionByName(mar_customers).show()

# TODO 1d: How many unique customers do you have across all three months?
all_unique = jan_customers.unionByName(feb_customers).unionByName(mar_customers).distinct()
all_unique.show()
...
# Bob and Diana
# Returning customers: Bob (customer_id=2), Diana (customer_id=4)
...
jan_customers.exceptAll(feb_customers).show()

# TODO 3b: Find NEW customers in February (not in January)
feb_customers.exceptAll(jan_customers).show()

# TODO 3c: Business question: What is the customer churn from Jan to Feb?
# Answer in a comment:
# customers in Jan who are not in Feb are Alice, Charlie. Churn rate is 50%
...
# Answer in a comment:
#  distinct removes duplicate ROWS where all columns match exactly
# dropDuplicates removes duplicates based only on the given column
